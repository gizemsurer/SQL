...
-----------------------View---------------------------------------------
--sorgunun saklanmýþ hali
--raporlama için sýk kullanýlan sorgular
--verinin güvenli paylaþýmý
--join yoksa insert içinde kullanýlabilþr
--kolon tekrarý yapýlmamalýdýr
--order by, top ile beraber kullanýlmalý
--içinde temp table kullanýlmaz
--allias ver

--create view VW_ViewIsmi
--with encryption,schemabinding(sorgu içinde kullanýlan tablolarý korur.Schema yazýlmasý zorunludur.) 
--as
--sorgular
--with check option 

--kargolanmamýþ ürünler
...
--Uzak makinadaki server üzerinden personellerin cirolarýný çek
...
---Kargolanmasý gecikmiþ sipariþlerin kaç gün geciktikleri ile beraber listesi
...
--en az satýlan 3 ürün
create view VW_EnAzSatýlanUcUrun
as
select top 3 
...
select*from VW_EnAzSatýlanUcUrun

--kategorilerin cirolarýnýn daðýlýmýný %oran olarak listeleyiniz
...
--1997 yýlý baz alýnarak ayný satýþlarýn 1998 yýlýnda da yapýlacaðý ön görülmüþtür.bu durumda 1998 yýlýnda satýþlarý ve stoðu çýkarýldýðýnda ürünlerden kaçar adet daha tedarik edilmesi gerektiðini listeleyiniz
--(ilgili hedefe ulaþmýþ olanlarýn listelenmemesi gerekmektedir.)
...
--od.ProductID,
--sum(od.Quantity) +
--(select UnitsInStock+UnitsOnOrder from Products where ProductID=od.productýd)Adet
...
--where productsýd=od.productsýd
...
-----------------------Function-----------------------------------------
--geri dönüþ zorunludur
--sorgu içinde kullanýlabilir(select clause)
...
--return donusdegeri
--end
--verilen iki metinsel ifadeyi aralarýnda bir boþluk olacak þekilde birleþtiren function yazýnýz
...
select dbo.Function_Son_KelimeAl('yeni bir hesap oluþtur')


---tedarikçilerin kontakt kiþilerinin soyadlarý nelerdir
select charindex('f','alafortanfoni',3)

-----------------------Stored Procedures--------------------------------
--geri dönüþ sabit deðer bazlý olamaz
--sorgu içinde kullanýlmaz
...
--ID si verilmiþ olan ürünün toplam satýþ adetini veren sp
...
print 'cevabý result ekranýna aktarýldý'
end
...
--çalýþanlarýn sadece ad,soyad ve yaþlarýný döndüren sp


--ID si verilen kategorinin içinde bulunan ürünlerin adýný listeleyen sp
--productname,unitprice,unitinstock deðerlerini alýp discontinued deðerini default olarak 0 atayýp insert iþlemi ypan sp
--ýd si verilen ürünü silen sp:(isdeleted true þeklinde)
--CategoryName ve ProductName verilerini alýp hem Categories hemde ona baðlý vaziyette products tablosuna insert yapan sp
